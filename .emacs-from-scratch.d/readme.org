...
**** TODO make line numbers increase in size at a lower rate.
**** TODO is this command useful? Try using it when line numbers weirdly reset. Or figure out why that happens!
...
#+begin_src emacs-lisp :lexical no
  (use-package avy
    ...
    :general
    (crj-leader-keys "la" '(avy-goto-char-timer :which-key "avy-goto-char-timer")))
