...
#+begin_src emacs-lisp
  (use-package evil
    ...
    :general
    (:keymaps 'normal :prefix "g"
                  "C-h" '(evil-title-case :which-key "Title-case operator.")))
#+end_src
...
**** Search With Avy Instead of Evil-Ex-Search

I find =avy-goto-char-timer= more useful generally for short-range cursor movements, and =consult-line= is generally more useful for me for long-ranges searches (especially with =orderless=), but I'll still fire up an =evil-ex-search= now and again.
...
#+begin_src emacs-lisp :lexical no
  (use-package avy
    ...
    :general
    (:keymaps '(evil-normal-state-map evil-motion-state-map) "/" #'avy-goto-char-timer)
    (:keymaps '(evil-normal-state-map evil-motion-state-map) "?" #'avy-goto-char-timer)
    (:states 'motion "\\" #'evil-ex-search-forward)
    (:states 'motion "M-\\" #'evil-ex-search-backward))
#+end_src
