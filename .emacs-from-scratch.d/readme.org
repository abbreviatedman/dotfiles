...
The short-range, simple Emacs commands are often easier to use when you don't want to switch to Normal State for editing and then back to Insert State to immediately get back to writing. Faster to type =C-w= to delete the word you're currnetly typing and then replace it, rather than =ESC ciw=. Or, worse, a situation where your editing doesn't bring you back to insert mode in the right place, like when the word /before/ your current word should be deleted—now you're talking about =ESC bb daw A= using Vim, but simply =M-b C-w C-e= with Vanilla Emacs bindings.
...
See the <S-backspace> mapping? Well, before I found how to write shift-backspace in the usual syntax, I couldn't for the life of me figure it out, and I happened upon this syntax in my research:

 =[(shift backspace)]=

 It's /not/ a short way to write it, but I'm agreeing more and more these days with the idea that short-hands are a real time-saver for when you're entering commands all by yourself (perfect example is installing someting globally via =npm= with =npm i -g [package name]= on your own machine by yourself), but if something's in a document, where someone (even just you several weeks later) needs to read and understand it, it should have the long-hand version (so you'd write it down as =npm install --global [package name]=).

 So! Maybe this is how these mappings should be written? =[(control d)]= instead of ="C-d"=? Maybe! Bears thinking about.
