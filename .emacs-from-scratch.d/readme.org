...
Only reversing the "move line up/down" and "next/previous error" keybindings to be more mnemonic.
...
This library adds some basic Evil bindings.
...
;; (use-package mu4e
;;   :init
;;   (setq mu4e-mu-binary "~/.emacs-from-scratch.d/straight/repos/mu/mu/mu")
...
#+begin_src emacs-lisp :tangle early-init.el
  (setq native-comp-deferred-compilation-deny-list '("rainbow"))
#+end_src
...
Making all your parens and curlies and square brackets /pretty/.

Auto-close parentheses, highlight the parens you're in, and switch to Smartparens for showing matching parens. (It's prettier.)

Also, automatically insert and indent a newline when putting a newline between empty curlies.
...
#+begin_src emacs-lisp
  (use-package highlight-parentheses
   ...
   (use-package emacs
     :init
     (defun crj--create-newline-and-enter-block (&rest _ignored)
     ...
     (sp-local-pair 'prog-mode "{" nil :post-handlers '((crj--create-newline-and-enter-block "RET"))))
#+end_src
...
******* TODO check what highlight-quoted mode does
