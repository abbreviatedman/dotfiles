...
**** TODO add full mixed-font experience
...
Then we make some keybindings for them:
...
Another fantastic efficient text editing library. This one exchanges two bits of text in an extremely Evilly way.
...
Remember recent files and update buffers if the file changes on disk.
...
#+begin_src emacs-lisp
  (defun my/buffer-face-mode-variable ()
    ...
    (interactive)
    (setq buffer-face-mode-face '(:family "IBM Plex Serif"
                                  :height 120
                                  :width normal))
    (buffer-face-mode))
