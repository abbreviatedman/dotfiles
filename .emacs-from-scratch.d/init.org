...
This is a bit of code taken from Emacs From Scratch.
...
First, some definitions of fonts by their use.

TODO explain the thinking behind coding with a variable pitch font.
...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (defvar crj--hooks-for-modes-without-line-numbers '(org-mode-hook
                                                        term-mode-hook
                                                        shell-mode-hook
                                                        treemacs-mode-hook
                                                        eshell-mode-hook))
...
Except don't use line numbers for for some modes.
...
(use-package emacs
  ...
  (defun crj--make-line-number-face-monospace (&rest args)
    (dolist (face crj--line-number-faces)
        (set-face-attribute face nil :family crj--fixed-pitch-font))
    args)
...
Like many Vimmers, I eventually realized that the things I personally liked about Vim were even more present in Emacs. Including Vim keybindings through Evil Mode, which are, in a lot of ways, /more Vim than Vim/. This is something I could argue at length, but... maybe not in this space. (Yet?)
...
I don't think this is the system I'll be using for keybindings, but it /is/ neat that I can define the leader-key keybinding for =crj--open-config-file= so easily!
...
(use-package general
  ...
  (crj--leader-key "fc" #'crj--open-config-file))

(defun crj--open-config-file ()
...
Modus Operandi is a fantastic theme for readability and accessiblity. It's about the uncoolest theme you can do, but... if you're using Emacs to be hip, you're doing it wrong.
